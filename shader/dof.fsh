...
float width = dimension.x; //texture width
float height = dimension.y; //texture height
...
float znear = clipping.x; //camera clipping start
float zfar = clipping.y; //camera clipping end
...
void main()
{
	float depth = linearize(texture2D(depthTexture, texcoords.xy).x);
	...
	vec2 noise = rand(texcoords.xy)*namount*blur;
    ...
    {
		col = texture2D(renderedTexture, texcoords.xy).rgb;
    }
	...
	else
	{
		...
		for (int i = 1; i <= rings; i += 1)
		{
			...
			for (int j = 0 ; j < ringsamples ; j += 1)
			{
				...
				col += color(texcoords.xy + vec2(pw*w,ph*h),blur)*mix(1.0,(float(i))/(float(rings)),bias)*p;
