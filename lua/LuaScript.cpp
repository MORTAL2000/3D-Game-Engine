...
LuaScript::LuaScript() : m_lua(0), m_filename("") {}

LuaScript::LuaScript(lua_State* lua, const std::string& filename) : m_lua(lua), m_filename(filename)
{
	reload();
}
...
bool LuaScript::reload()
{
	...
	std::string source = FileReader::read(m_filename);
	if(source.empty()) {
