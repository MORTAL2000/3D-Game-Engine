...
rect_t CFont::render(const std::string& text, const vec2& position, const vec2& viewport)
{
	...
	glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(vertex2_t), VBO_OFFSET(0, vertex2_t, position));
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(vertex2_t), VBO_OFFSET(0, vertex2_t, texcoord));
