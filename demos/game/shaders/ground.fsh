...
float compare(sampler2D map, vec4 coords) {
	...
	for(int x = -2; x <= 2; x++) {
		for(int y = -2; y <= 2; y++) {
            ...
            vec4 offset = vec4(float(x) + rnd.x, float(y) + rnd.y, 0.0, 0.0) * texel;
...
void main() {
	...
	float diffuseFactor = max(dot(sunDirection, normal), 0.0);
    ...
    if(textured > 0.0) retColor *= tex * 2.5;

    //shadow stuff
    float visibility = compare(shadowMap, shadowCoord);
    //float visibility = getShadowFactor(shadowMap, shadowCoord);

    // night 'torch' and anti-shadow
    float dist = length(worldSpacePosition - cameraPosition);
    float cosL = max(0.0, dot(-normalize(worldSpacePosition - cameraPosition), fragmentNormal.xyz));
    float att = 1.0 / (dist * dist * 0.002);
    if(sunE <= 0.0) {
        retColor = clamp(retColor * att * cosL, 0.0, 1.0);
        visibility = 1.0;
    }
	...
	//final apply
	gl_FragColor = fragment;//fog(fragment);
